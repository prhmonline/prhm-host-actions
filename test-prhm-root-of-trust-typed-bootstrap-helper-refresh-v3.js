...
const mod=require('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v3.js');
...
test('source excludes generic execution and unrelated product paths',()=>{const fs=require('node:fs');const s=fs.readFileSync(require.resolve('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v3.js'),'utf8');for(const bad of ['/home/drtarjomeh','/home/imotion','mysql','node:pg','child_process.exec(','SSH_ORIGINAL_COMMAND','authorized_keys'])assert.equal(s.includes(bad),false,bad)});
