...
function rahkomakRegistrationState(m){
 const actual=Object.fromEntries(Object.entries(REG_OWNER_PATHS).map(([n,p])=>[n,fileSha(p)]));
 const pre=Object.keys(m.RAHEKOMAK_REGISTRATION_BASELINE_SHA256).every(n=>actual[n]===m.RAHEKOMAK_REGISTRATION_BASELINE_SHA256[n]);
 const post=Object.keys(m.RAHEKOMAK_REGISTRATION_CANDIDATE_SHA256).every(n=>actual[n]===m.RAHEKOMAK_REGISTRATION_CANDIDATE_SHA256[n]);
 return {actual,pre,post};
}
function installerForContract(m){
 const state=rahkomakRegistrationState(m);
 if(state.pre)return m.buildRegistrationInstallerSource();
 assert.equal(state.post,true,'RahKomak registration owners must be exact pre-state or post-state');
