...
const mod=require('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v7.js');
...
test('fixed identity and zero-input target contract',()=>{assert.equal(mod.TARGET,'/opt/prhm-agent-selfmaint-exec/actions/control-plane-typed-bootstrap-transport-v1.js');assert.equal(mod.PREIMAGE_SHA,'80ce1b2d3a53d45a750035a2ff5c8c67f1e31695e08fd41c8f980d7c7109725a');assert.equal(mod.CANDIDATE_SHA,'3388ae690dcb3bf8b5fb2da8d9e489e5169f298d1bc4d74e7d6d66403eba9bf9');assert.equal(sha(mod.candidateBytes()),mod.CANDIDATE_SHA)});
...
test('source excludes generic execution and unrelated product paths',()=>{const fs=require('node:fs');const s=fs.readFileSync(require.resolve('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v7.js'),'utf8');for(const bad of ['/home/drtarjomeh','/home/imotion','mysql','node:pg','child_process.exec(','SSH_ORIGINAL_COMMAND','authorized_keys'])assert.equal(s.includes(bad),false,bad)});
