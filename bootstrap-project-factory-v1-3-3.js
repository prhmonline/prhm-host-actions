...
function main(){
  ...
  try{
    ...
    const e2e=run(NODE,[TARGET,'--manifest-base64',Buffer.from(JSON.stringify(manifest)).toString('base64')],{timeout:900000});assertZero(e2e,'e2e');if(!String(e2e.stdout||'').includes('PRHM_BOOTSTRAP_OK='+root))die('e2e_success_marker_missing');
