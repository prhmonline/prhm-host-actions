...
test('buildPreflightReport emits only secret-safe allowlisted metadata',()=>{
  const report=buildPreflightReport({envSha256:'a'.repeat(64),portKey:'PORT',currentPort:8140,replacementPort:8141,recoveryHealthy:true,agentApiHealthy:true,mcpHealthy:true,envText:'PORT=8140\nTOKEN=super-secret-fixture\n',token:'super-secret-fixture'});
...
test('expectedEndpoints are fixed and contain no user-supplied surface',()=>{
  assert.deepEqual(expectedEndpoints,{recovery:'http://10.71.0.118:8140/health',bridge:'http://127.0.0.1:8141/health',bridgePrivate:'http://10.71.0.118:8141/health',agentApi:'http://127.0.0.1:8099/health',mcp:['http://127.0.0.1:8123/health','http://127.0.0.1:8124/health','http://127.0.0.1:8125/health']});
});
...
test('preflight is read-only, SHA-binds env, and verifies fixed dependencies',async()=>{
  const deps=fakeDeps();
  const out=await preflight(deps);
  assert.equal(out.env_sha256,sha('PORT=8140\nTOKEN=fixture-secret-value\n'));
  assert.equal(out.current_port,8140);
  assert.equal(out.replacement_port,8141);
  assert.deepEqual(deps.events,[]);
});

test('apply backs up before write, restarts only bridge, and verifies stable health',async()=>{
  const deps=fakeDeps();
  const out=await apply(deps);
  assert.equal(out.ok,true);
  assert.equal(out.rollback_performed,false);
  assert.equal(out.database_mutation,false);
  assert.equal(out.recovery_mutation,false);
  assert.equal(deps.getCurrent(),'PORT=8141\nTOKEN=fixture-secret-value\n');
  assert.deepEqual(deps.events,['backup','write','restart:bridge','sleep']);
});

test('apply aborts before mutation when env SHA drifts after preflight',async()=>{
  let reads=0;
  const deps=fakeDeps({readEnv:()=>{reads++;return Buffer.from(reads===1?'PORT=8140\nTOKEN=fixture-secret-value\n':'PORT=8140\nTOKEN=changed-after-preflight\n');}});
  await assert.rejects(()=>apply(deps),/env_sha_drift_before_apply/);
  assert.deepEqual(deps.events,[]);
});

test('post-write verification failure restores exact preimage and restarts only bridge',async()=>{
  let bridgeChecks=0;
  const deps=fakeDeps({health:(url)=>{
    if(url===expectedEndpoints.bridge){bridgeChecks++;return bridgeChecks===1?{ok:false,service:'prhm-readonly-http'}:{ok:true,service:'prhm-readonly-http'};}
    if(url===expectedEndpoints.recovery)return {ok:true,service:'prhm-recovery-agent'};
    if(url===expectedEndpoints.agentApi)return {ok:true,service:'ssh-agent-api'};
    if(expectedEndpoints.mcp.includes(url))return {ok:true,service:'prhm-dev-agent-mcp'};
    return null;
  }});
  await assert.rejects(()=>apply(deps),/readonly_bridge_repair_failed_rolled_back:bridge_health_failed/);
  assert.equal(deps.getCurrent(),'PORT=8140\nTOKEN=fixture-secret-value\n');
  assert.deepEqual(deps.events,['backup','write','restart:bridge','write','restart:bridge']);
});

test('restart counter increase after health causes rollback',async()=>{
  const deps=fakeDeps();
  let calls=0;
  deps.getNRestarts=()=>{calls++;return calls===1?41:42;};
  await assert.rejects(()=>apply(deps),/readonly_bridge_repair_failed_rolled_back:restart_counter_increased/);
  assert.equal(deps.getCurrent(),'PORT=8140\nTOKEN=fixture-secret-value\n');
});
