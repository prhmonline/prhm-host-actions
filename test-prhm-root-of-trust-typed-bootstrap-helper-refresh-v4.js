...
const mod=require('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v4.js');
...
test('fixed identity and zero-input target contract',()=>{assert.equal(mod.TARGET,'/opt/prhm-agent-selfmaint-exec/actions/control-plane-typed-bootstrap-transport-v1.js');assert.equal(mod.PREIMAGE_SHA,'3f0712fbadeb94d792911419fc71d220c5b135f82510b52b49e6d08ee6199885');assert.equal(mod.CANDIDATE_SHA,'1a789eb89edfdeb15ddf7c645dd34b33cbe8af35ec9469b015fba94635742f8e');assert.equal(sha(mod.candidateBytes()),mod.CANDIDATE_SHA)});
...
test('source excludes generic execution and unrelated product paths',()=>{const fs=require('node:fs');const s=fs.readFileSync(require.resolve('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v4.js'),'utf8');for(const bad of ['/home/drtarjomeh','/home/imotion','mysql','node:pg','child_process.exec(','SSH_ORIGINAL_COMMAND','authorized_keys'])assert.equal(s.includes(bad),false,bad)});
