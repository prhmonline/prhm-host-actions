...
const mod=require('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v2.js');
...
test('fixed identity and zero-input target contract',()=>{assert.equal(mod.TARGET,'/opt/prhm-agent-selfmaint-exec/actions/control-plane-typed-bootstrap-transport-v1.js');assert.equal(mod.PREIMAGE_SHA,'d4330d3818c4a392ad62acf92b282f2d86fbd77a31dd0642dbc32aeab54d919c');assert.equal(mod.CANDIDATE_SHA,'f80273d9e273c5b35910b5d85575caf435bcff6aa4230f760380c82fb6e5a839');assert.equal(sha(mod.candidateBytes()),mod.CANDIDATE_SHA)});
...
test('source excludes generic execution and unrelated product paths',()=>{const fs=require('node:fs');const s=fs.readFileSync(require.resolve('./bootstrap-prhm-root-of-trust-typed-bootstrap-helper-refresh-v2.js'),'utf8');for(const bad of ['/home/drtarjomeh','/home/imotion','mysql','node:pg','child_process.exec(','SSH_ORIGINAL_COMMAND','authorized_keys'])assert.equal(s.includes(bad),false,bad)});
