...
function registryPid(){const r=exec('/usr/bin/systemctl',['show','prhm-company-registry.service','-p','MainPID','--value']);const pid=String(r.stdout||'').trim();if(!/^\d+$/.test(pid)||pid==='0')fail('registry_pid_unavailable');return pid}
function rawHealth(){const r=cp.spawnSync('/usr/bin/nsenter',['-t',registryPid(),'-n','/usr/bin/curl','-fsS','--max-time','2','http://127.0.0.1:18133/health'],{encoding:'utf8'});if(r.status!==0)return null;try{return JSON.parse(r.stdout)}catch{return null}}
