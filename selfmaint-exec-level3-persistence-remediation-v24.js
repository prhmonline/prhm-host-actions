...
const ACTION='selfmaint_exec_level3_persistence_remediation_v24';
...
const BACKUP_ROOT='/var/backups/prhm-selfmaint-exec-level3-persistence-v24';
const RESULT='/var/lib/prhm-agent-selfmaint-exec/selfmaint-exec-level3-persistence-v24/latest.json';
...
function healthCheck(){
  const raw=cp.execFileSync('/usr/bin/curl',['-fsS','--unix-socket',SOCKET,'http://localhost/health'],{encoding:'utf8',timeout:15000,maxBuffer:262144,env:{PATH:'/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',LC_ALL:'C',HOME:'/nonexistent'}});
  let j;try{j=JSON.parse(raw)}catch{fail('health_invalid_json')}
  if(j?.ok!==true||j?.service!=='prhm-agent-selfmaint-exec')fail('health_not_ok');
  return {ok:true,service:j.service,version:j.version||null};
}
