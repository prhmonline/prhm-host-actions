...
const ACTION='agent_mcp_blue_rolling_refresh_v4';
...
const PATHS=Object.freeze({
  ...
  backupRoot:'/var/backups/prhm-agent-mcp-blue-rolling-refresh-v4',
  latest:'/var/backups/prhm-agent-mcp-blue-rolling-refresh-v4/latest.json'
});
...
const EXPECTED_SHA=Object.freeze({
  ...
  [PATHS.hostActionsV2]:'d9823fe703c91c938cdf688a6c0944775fc5226a420892c3a35053170a19f360',
...
function refuseMcpServiceContext(){
  ...
  if(String(cgroup).split('\n').some(line=>/(?:^|\/)prhm-agent-mcp(?:-(?:green|blue))?\.service(?:\/|$)/.test(line)))throw new Error('refuse_mcp_service_context');
}
...
function pointerInfo(){
  if(!fs.existsSync(PATHS.pointer))return{regular:false,green:false,blue:false,bytes:null,meta:null};
  const s=fs.lstatSync(PATHS.pointer);if(!s.isFile()||s.isSymbolicLink())return{regular:false,green:false,blue:false,bytes:null,meta:null};
  const bytes=fs.readFileSync(PATHS.pointer);const text=bytes.toString('utf8');
  return{regular:true,green:/^8124\n?$/.test(text),blue:/^8125\n?$/.test(text),bytes,meta:{mode:s.mode&0o777,uid:s.uid,gid:s.gid}};
}
function topologyContract(){
  const router=unitShow(UNITS.router),green=unitShow(UNITS.green),blue=unitShow(UNITS.blue);
  ...
  return{match:routerContract&&backend(green,UNITS.green)&&backend(blue,UNITS.blue),router,green,blue};
}
async function inspectProduction(){
  ...
  return{
    ...
    green_enabled:topo.green.UnitFileState==='enabled',
    blue_active:topo.blue.ActiveState==='active'&&topo.blue.SubState==='running',
    blue_disabled:topo.blue.UnitFileState==='disabled',
...
function validatePreflightState(s){
  ...
  if(s.green_active!==true||s.green_enabled!==true)throw new Error('green_state_invalid');
  if(s.blue_active!==true||s.blue_disabled!==true)throw new Error('blue_state_invalid');
...
async function preflight(adapter){
  ...
  return{ok:true,action:ACTION,preflight_only:true,production_mutation:false,database_mutation:false,api_mutation:false,router_restart_reload:false,green_restart_stop:false,current_backend:PORTS.green,candidate_backend:PORTS.blue,source_sha_match:true,topology_match:true,blue_health:true,public_health:true,green_health:true};
}
...
class ProductionAdapter{
  ...
  async captureApplyState(){
    ...
    const state={backup_path:dir,pointer_backup:before,pointer_mode:ptr.meta.mode,pointer_uid:ptr.meta.uid,pointer_gid:ptr.meta.gid,blue_pre_active:bluePreActive,blue_pre_enabled:bluePreEnabled,previous_backend:PORTS.green,source_sha:Object.fromEntries(Object.entries(EXPECTED_SHA).map(([f])=>[f,shaFile(f)]))};
  ...
  async switchToBlue(){const ptr=pointerInfo();if(!ptr.regular||!ptr.green)throw new Error('pointer_changed_before_cutover');atomicBuffer(PATHS.pointer,Buffer.from('8125\n'),ptr.meta);}
  ...
  async restoreApplyState(pre){
    ...
    if(pre.blue_pre_active!==true){try{systemctl('stop',UNITS.blue);}catch{}}
  ...
  async verifyRollbackState(e){
    if(!sourceShaMatch())return false;const ptr=pointerInfo();if(!ptr.regular||!ptr.blue)return false;
    if(!e.pointer_backup||!fs.existsSync(e.pointer_backup)||!safeRegular(e.pointer_backup))return false;
    return isActive(UNITS.router)&&isActive(UNITS.green)&&isActive(UNITS.blue);
  }
  ...
  async restoreBluePrestate(e){if(e.blue_pre_active!==true){try{systemctl('stop',UNITS.blue);}catch{}}systemctl(e.blue_pre_enabled===true?'enable':'disable',UNITS.blue);}
  ...
  async verifyFinalizeState(){const ptr=pointerInfo();return sourceShaMatch()&&ptr.regular&&ptr.blue&&isActive(UNITS.router)&&isActive(UNITS.green)&&isActive(UNITS.blue)&&await endpointOk(PORTS.blue,'/health','ok')&&await endpointOk(PORTS.blue,'/ready','ready');}
