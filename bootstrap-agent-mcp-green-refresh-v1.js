...
const ACTION='agent_mcp_green_refresh_v1';
...
const PATHS=Object.freeze({
  ...
  backupRoot:'/var/backups/prhm-agent-mcp-green-refresh',
  latest:'/var/backups/prhm-agent-mcp-green-refresh/latest.json'
});
...
const EXPECTED_SHA=Object.freeze({
  ...
  [PATHS.safeFiles]:'87da44a939478786b9a48585c1cccacd862b683831dbba976d8b6a85869d2473',
...
function sourceShaMatch(){for(const [file,sha] of Object.entries(EXPECTED_SHA)){if(!fs.existsSync(file)||!safeRegular(file)||shaFile(file)!==sha)return false;}return true;}
...
async function inspectProduction(){
  ...
  return{
    hostname,
    source_sha_match:sourceShaMatch(),
    ...
    green_inactive:topo.green.ActiveState==='inactive',
...
function validatePreflightState(s){
  ...
  if(s.source_sha_match!==true)throw new Error('source_sha_mismatch');
  ...
  if(s.green_inactive!==true||s.green_disabled!==true)throw new Error('green_prestate_invalid');
  ...
  if(s.green_free!==true)throw new Error('green_port_busy');
  ...
  if(s.pointer_blue!==true)throw new Error('pointer_not_blue');
...
async function preflight(adapter){
  ...
  return{ok:true,action:ACTION,preflight_only:true,production_mutation:false,database_mutation:false,api_mutation:false,router_restart_reload:false,blue_restart_stop:false,current_backend:PORTS.blue,candidate_backend:PORTS.green,source_sha_match:true,topology_match:true,green_free:true,public_health:true,blue_health:true};
}
async function runApply(adapter){
  await preflight(adapter);let pre=null;let mutated=false;
  try{
    pre=await adapter.captureApplyState();
    await adapter.startGreen();mutated=true;
    if(await adapter.healthGreen()!==true)throw new Error('green_health_failed');
    if(await adapter.readyGreen()!==true)throw new Error('green_ready_failed');
    await adapter.switchToGreen();
    ...
    const out={ok:true,action:ACTION,preflight_only:false,production_mutation:true,database_mutation:false,api_mutation:false,router_restart_reload:false,blue_restart_stop:false,previous_backend:PORTS.blue,active_backend:PORTS.green,green_started:true,public_health:true,rollback_performed:false,backup_path:pre.backup_path};
...
function validApplyEvidence(e){return e&&e.status==='applied'&&e.active_backend===PORTS.green&&e.previous_backend===PORTS.blue&&e.finalized!==true&&e.rolled_back!==true;}
async function runRollback(adapter){
  ...
  await adapter.restoreGreenPrestate(e);
  const out={ok:true,action:ACTION,production_mutation:true,database_mutation:false,api_mutation:false,active_backend:PORTS.blue,rollback_performed:true,finalized:false};
...
async function runFinalize(adapter){
  ...
  await adapter.enableGreen();await adapter.disableBlue();
  ...
  const out={ok:true,action:ACTION,production_mutation:true,database_mutation:false,api_mutation:false,active_backend:PORTS.green,rollback_performed:false,finalized:true,blue_still_running:true};
...
class ProductionAdapter{
  ...
  async captureApplyState(){
    ...
    const ptr=pointerInfo();if(!ptr.regular||!ptr.blue)throw new Error('pointer_prestate_changed');
    const greenPreActive=isActive(UNITS.green),greenPreEnabled=isEnabled(UNITS.green);
    const before=path.join(dir,'mcp-active.before');atomicBuffer(before,ptr.bytes,{mode:0o600});
    const state={backup_path:dir,pointer_backup:before,pointer_mode:ptr.meta.mode,pointer_uid:ptr.meta.uid,pointer_gid:ptr.meta.gid,green_pre_active:greenPreActive,green_pre_enabled:greenPreEnabled,previous_backend:PORTS.blue,source_sha:Object.fromEntries(Object.entries(EXPECTED_SHA).map(([f])=>[f,shaFile(f)]))};
    atomicJson(path.join(dir,'pre-state.json'),state);this.applyState=state;return state;
  }
  async startGreen(){systemctl('start',UNITS.green);}
  async healthGreen(){return waitEndpoint(PORTS.green,'/health','ok');}
  async readyGreen(){return waitEndpoint(PORTS.green,'/ready','ready');}
  async switchToGreen(){const ptr=pointerInfo();if(!ptr.regular||!ptr.blue)throw new Error('pointer_changed_before_cutover');atomicBuffer(PATHS.pointer,Buffer.from('8125\n'),ptr.meta);}
  ...
  async persistApply(out,pre){
    const record={status:'applied',action:ACTION,backup_path:pre.backup_path,pointer_backup:pre.pointer_backup,pointer_mode:pre.pointer_mode,pointer_uid:pre.pointer_uid,pointer_gid:pre.pointer_gid,green_pre_active:pre.green_pre_active,green_pre_enabled:pre.green_pre_enabled,previous_backend:PORTS.blue,active_backend:PORTS.green,finalized:false,rolled_back:false,applied_at:new Date().toISOString(),result:redactEvidence(out)};
  ...
  async restoreApplyState(pre){
    ...
    if(pre.green_pre_active!==true){try{systemctl('stop',UNITS.green);}catch{}}
    try{systemctl(pre.green_pre_enabled===true?'enable':'disable',UNITS.green);}catch{}
  ...
  async verifyRollbackState(e){
    if(!sourceShaMatch())return false;const ptr=pointerInfo();if(!ptr.regular||!ptr.green)return false;
  ...
  async restoreGreenPrestate(e){if(e.green_pre_active!==true){try{systemctl('stop',UNITS.green);}catch{}}systemctl(e.green_pre_enabled===true?'enable':'disable',UNITS.green);}
  async persistRollback(out,e){const next={...e,status:'rolled_back',active_backend:PORTS.blue,rolled_back:true,finalized:false,rolled_back_at:new Date().toISOString(),rollback_result:redactEvidence(out)};atomicJson(path.join(e.backup_path,'rollback-result.json'),next);atomicJson(PATHS.latest,next);}
  async verifyFinalizeState(){const ptr=pointerInfo();return sourceShaMatch()&&ptr.regular&&ptr.green&&isActive(UNITS.router)&&isActive(UNITS.blue)&&isActive(UNITS.green)&&await endpointOk(PORTS.green,'/health','ok')&&await endpointOk(PORTS.green,'/ready','ready');}
  async enableGreen(){systemctl('enable',UNITS.green);}
  async disableBlue(){systemctl('disable',UNITS.blue);}
  async persistFinalize(out,e){const next={...e,status:'finalized',active_backend:PORTS.green,finalized:true,rolled_back:false,finalized_at:new Date().toISOString(),finalize_result:redactEvidence(out)};atomicJson(path.join(e.backup_path,'finalize-result.json'),next);atomicJson(PATHS.latest,next);}
}
...
module.exports={ACTION,PORTS,PATHS,UNITS,EXPECTED_SHA,parseMode,redactEvidence,preflight,runApply,runRollback,runFinalize,ProductionAdapter};
