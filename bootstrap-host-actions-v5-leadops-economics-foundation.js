...
function main(){
  ...
  syntaxCheck(FILES.base,candidate.base,false);
  ...
  const preflightReport={
    ...
    candidate_hashes:{
      base:shaBuffer(Buffer.from(candidate.base)),
  ...
  for(const key of ['base','executor','plugin','policy','v4helper']){
  ...
  try{
    ...
    if(base.ok!==true||base.version!=='1.0.0-l4-fail-closed')
      fail('base_selfmaint_health_invalid:'+JSON.stringify(base));
    ...
    if(!fs.readFileSync(FILES.base,'utf8').includes(ACTION))fail('action_missing_from_base');
    ...
    const result={
      ...
      base_selfmaint_sha256:shaFile(FILES.base),
