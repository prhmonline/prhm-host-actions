...
const EXPECTED_SHA=Object.freeze({
  ...
  [PATHS.mcpSource]:'9034957651c7f367b6c9f5c5db79962c5cc86d5201d2ec5855c64458f6117a5b'
});
...
async function applyLane(kind,adapter,state){
  ...
  try{
    ...
    if(await adapter.candidateReady(kind,pre)!==true)throw new Error(`${kind}_candidate_ready_failed`);
      if(kind==='mcp'){
        if(typeof adapter.candidateRuntimePreflight!=='function')throw new Error('mcp_candidate_runtime_preflight_unavailable');
        if(await adapter.candidateRuntimePreflight(kind,pre)!==true)throw new Error('mcp_candidate_runtime_preflight_failed');
      }
      await adapter.assertPointerUnchanged(kind,pre);
    await adapter.enableCandidate(kind,pre);
if(await adapter.candidateEnabled(kind,pre)!==true)throw new Error(`${kind}_candidate_enable_failed`);
...
function validEvidence(e){if(!e||e.ok!==true||e.partial_success===true||e.rollback_performed===true||e.finalized===true||e.rolled_back===true||!e.api||!e.mcp||e.api.status!=='applied'||e.mcp.status!=='applied'||e.api.rollback_performed===true||e.mcp.rollback_performed===true)throw new Error('apply_evidence_invalid');return e;}
async function runRollback(adapter){
  const e=validEvidence(await adapter.loadLatestEvidence());const out={ok:true,action:ACTION,schema_version:'prhm.host-action-result.v1',production_mutation:true,database_mutation:false,rollback_performed:true,finalized:false,api:e.api,mcp:e.mcp};
...
async function runFinalize(adapter){
  const e=validEvidence(await adapter.loadLatestEvidence());const out={ok:true,action:ACTION,schema_version:'prhm.host-action-result.v1',production_mutation:true,database_mutation:false,rollback_performed:false,finalized:true,api:e.api,mcp:e.mcp};
...
class ProductionAdapter{
  ...
  async candidateReady(kind,pre){return waitEndpoint(pre.candidate_backend,'/ready','ready');}
    async candidateRuntimePreflight(kind,pre){
      if(kind!=='mcp')return true;
      if(!safeRegular(MCP_RUNTIME_PREFLIGHT))throw new Error('mcp_runtime_preflight_artifact_missing');
      if(shaFile(MCP_RUNTIME_PREFLIGHT)!==MCP_RUNTIME_PREFLIGHT_SHA)throw new Error('mcp_runtime_preflight_sha_mismatch');
      const ptr=pointerInfo('mcp');
      if(!ptr.regular||ptr.port!==pre.previous_backend)throw new Error('pointer_changed_before_runtime_preflight');
      command(MCP_RUNTIME_PREFLIGHT,[String(pre.candidate_backend)],60000);
      const after=pointerInfo('mcp');
      if(!after.regular||after.port!==pre.previous_backend)throw new Error('pointer_changed_during_runtime_preflight');
      return true;
    }
  async enableCandidate(kind,pre){systemctl('enable',UNITS[kind][pre.candidate_slot]);}
async candidateEnabled(kind,pre){return isEnabled(UNITS[kind][pre.candidate_slot]);}
