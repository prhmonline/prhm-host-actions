...
function verifyTargetRoot(root=TARGET.root){if(!fs.existsSync(root)||!directory(root))fail('target_root_missing_or_unsafe');const config=path.join(root,'wp-config.php'),content=path.join(root,'wp-content');if(!fs.existsSync(config)||!regular(config))fail('wordpress_config_missing_or_unsafe');if(!fs.existsSync(content)||!directory(content))fail('wordpress_content_missing_or_unsafe');return{target_root_exists:true,wordpress_detected:true};}
...
function preflight(){const targetEvidence=verifyTargetRoot();verifyBaselines();const c=buildCandidates();return{ok:true,schema_version:'prhm.host-action-preflight.v1',action:ACTION,preflight_only:true,target:TARGET,...targetEvidence,candidate_sha256:c.hashes,requires_zdt_refresh:true,production_site_mutation:false,database_mutation:false,redirect_mutation:false,canonical_mutation:false,deploy:false,external_network:false,token_read:false};}
...
function apply(){const started_at=new Date().toISOString(),targetEvidence=verifyTargetRoot();verifyBaselines();const c=buildCandidates(),backup=backupOriginals(),mutated=[];try{
  ...
  const result={ok:true,schema_version:'prhm.host-action-result.v1',action:ACTION,started_at,finished_at:new Date().toISOString(),target:TARGET,...targetEvidence,registered_in_source:true,requires_zdt_refresh:true,candidate_sha256:c.hashes,backup_dir:backup.dir,production_site_mutation:false,database_mutation:false,redirect_mutation:false,canonical_mutation:false,deploy:false,external_network:false,token_read:false,rollback_performed:false,rollback:{performed:false}};
...
module.exports={ACTION,TARGET,PATHS,EXPECTED,sha,verifyTargetRoot,injectImotionMarketingProject,patchAgentApiServer,patchProjectPlugin,patchSafeFiles,patchZdtManifest,preflight,apply};
