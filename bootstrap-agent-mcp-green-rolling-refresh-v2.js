...
const ACTION='agent_mcp_green_rolling_refresh_v2';
...
const PATHS=Object.freeze({
  ...
  backupRoot:'/var/backups/prhm-agent-mcp-green-rolling-refresh-v2',
  latest:'/var/backups/prhm-agent-mcp-green-rolling-refresh-v2/latest.json'
});
...
const SAFEFILES_SHA=Object.freeze({
  v3:'92e6e279f10fe8561a9c986d9dc90d6bc0cd284009ed9984f51fefe202ac6252',
...
function sourceReadyV4(){return coreShaMatch()&&safeFilesSha()===SAFEFILES_SHA.v4;}
...
function validatePreflightState(s){
  ...
  if(![SAFEFILES_SHA.v3,SAFEFILES_SHA.v4].includes(s.safe_files_sha))throw new Error('safe_files_sha_mismatch');
...
async function preflight(adapter){
  ...
  const ready=s.safe_files_sha===SAFEFILES_SHA.v4;
  return{ok:true,action:ACTION,preflight_only:true,production_mutation:false,database_mutation:false,api_mutation:false,router_restart_reload:false,blue_restart_stop:false,current_backend:PORTS.blue,candidate_backend:PORTS.green,source_state:ready?'v4_ready':'v3_prepatch',source_patch_required:!ready,apply_ready:ready,topology_match:true,public_health:true,blue_health:true,green_health:true};
}
async function runApply(adapter){
  const gate=await preflight(adapter);if(gate.apply_ready!==true)throw new Error('source_v4_not_ready');let pre=null;let mutated=false;
  try{
...
class ProductionAdapter{
  ...
  async captureApplyState(){
    ...
    const state={backup_path:dir,pointer_backup:before,pointer_mode:ptr.meta.mode,pointer_uid:ptr.meta.uid,pointer_gid:ptr.meta.gid,green_pre_active:greenPreActive,green_pre_enabled:greenPreEnabled,previous_backend:PORTS.blue,source_sha:{...Object.fromEntries(Object.entries(EXPECTED_SHA).map(([f])=>[f,shaFile(f)])),[PATHS.safeFiles]:safeFilesSha()}};
  ...
  async restoreApplyState(pre){
    ...
    try{systemctl(pre.green_pre_active===true?'start':'stop',UNITS.green);}catch{}
  ...
  async verifyRollbackState(e){
    if(!sourceReadyV4())return false;const ptr=pointerInfo();if(!ptr.regular||!ptr.green)return false;
  ...
  async restoreGreenPrestate(e){try{systemctl(e.green_pre_active===true?'start':'stop',UNITS.green);}catch{};systemctl(e.green_pre_enabled===true?'enable':'disable',UNITS.green);}
  ...
  async verifyFinalizeState(){const ptr=pointerInfo();return sourceReadyV4()&&ptr.regular&&ptr.green&&isActive(UNITS.router)&&isActive(UNITS.blue)&&isActive(UNITS.green)&&await endpointOk(PORTS.green,'/health','ok')&&await endpointOk(PORTS.green,'/ready','ready');}
