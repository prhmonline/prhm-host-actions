...
if(require.main===module){
 try{refreshApprovalOnce();const bytes=loadBase();const compiled=new Module(__filename,module);compiled.filename=path.join(__dirname,'server.approval-center-policy-refresh-once-v1.js');compiled.paths=module.paths;compiled._compile(bytes.toString('utf8'),compiled.filename)}catch(e){process.stderr.write(String(e&&e.stack||e)+'\n');process.exit(1)}
}
module.exports={BASE_SHA,POLICY_PATH,POLICY_SHA,APPROVAL_SERVER_PATH,APPROVAL_SERVER_SHA,POLICY_VERSION,SERVICE,EXPECTED_OLD_APPROVAL_PID,HEALTH_URL,REQUIRED_OPERATION,REQUIRED_TOOL,REQUIRED_ACTION,validatePolicyObject,validatePolicyBytes,baseline,refreshApprovalOnce,productionDeps,loadBase};
