...
test('packaged adapter unit keeps Node jitless under MemoryDenyWriteExecute',()=>{
  const unit=fs.readFileSync(path.join(dir,'prhm-deployhq-control.service'),'utf8');
  assert.match(unit,/^ExecStart=\/usr\/local\/bin\/prhm-node --jitless \/opt\/prhm-deployhq-control\/server\.js --serve$/m);
  assert.match(unit,/^MemoryDenyWriteExecute=true$/m);
});

test('adapter entrypoint is world-readable/executable for dedicated service user',()=>{const r=manifest.records.find(x=>x.destination_path==='/opt/prhm-deployhq-control/server.js');assert.ok(r);assert.equal(r.mode,'0755')});
